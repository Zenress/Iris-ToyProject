...
#Making column header names so that the dataset can be split accordingly
...
#Reading the dataset with no headers and adding the column names i made above as the column names
...
def train_model(dtc,iris_kfold_n5):
    ...
    for train_index, test_index in iris_kfold_n5.split(iris_df,iris_df[cfg["column_names"]["column_nr5"]]):
        #Splitting the dataset between 4 variables to do training and testing
